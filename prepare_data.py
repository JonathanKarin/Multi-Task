...
def get_y():
    ...
    for line in f:
        if row > 0:
            ...
            if next_row[0] == 'SetA':
                del next_row[0:3]
                data_train.append(next_row)
            else:
                del next_row[0:3]
                data_test.append(next_row)
...
def save_data_target():
    ...
    for index, row in Test.iterrows():
        y_shaped_b[i] = row[1:]
        i = i + 1
    np.save('y_shaped_a.npy', y_shaped_a)
    np.save('y_shaped_b.npy', y_shaped_b)
    return
...
def save_data_b():
    ...
    for index, row in Test.iterrows():
        ...
        y_shaped_b[i] = row[1:]
        i = i + 1
    np.save('x_shaped_b.npy', x_shaped_b)
    np.save('y_shaped_b.npy', y_shaped_b)
    np.save('x_struct_b.npy', x_struct_b)
    return
